...
class FilterView(ListView):
    ...
    queryset = GameModel.objects.filter(eu_sales__range=range(1, 3))#name__endswith='m')#na_sales__lte=0.7) #name__in=['Hitman', 'Tetris']) #name__contains='Hitman')
...
class ValuesView(ListView):
    ...
    queryset = GameModel.objects.filter(name="Hitman").values("name",
                                                              "platform",
                                                              "year")
